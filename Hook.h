	...
	{
		{
			if (TargetSilientHeli != NULL)
			{
				...
				const auto Attack = safe_read(PlayerAttack + 0x18, uintptr_t);
				int health = (int)ceil(safe_read(TargetSilientHeli + 0x20C, float)); // Player_Health ,// public float attackSpacing;
				if (health <= 5000)
				{
...
inline void InitHook()
{
	...
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x715E40)), reinterpret_cast<void**>(&original_sound), CustomHitsound);//public void DoHitNotify(HitInfo info) { }
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x214B670)), reinterpret_cast<void**>(&original_consolerun), Run);//&public static string Run(ConsoleSystem.Option options, string strCommand, object[] args) { }
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x6CA1E0)), reinterpret_cast<void**>(&original_create_projectile), CreateProjectile);// private Projectile CreateProjectile(string prefabPath, Vector3 pos, Vector3 forward, Vector3 velocity) { }
	...
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x44F8C0)), reinterpret_cast<void**>(&original_canholditems), CanHoldItems);
